...
type Load interface {
	AddOperatorBundle(bundle *manifests.Bundle) error
	AddBundlePackageChannels(manifest manifests.PackageManifest, bundle manifests.Bundle) error
	AddPackageChannels(manifest manifests.PackageManifest) error
...
type Query interface {
	...
	GetPackage(ctx context.Context, name string) (*manifests.PackageManifest, error)
	...
	GetChannelEntriesThatReplace(ctx context.Context, name string) (entries []*manifests.ChannelEntry, err error)
	...
	GetChannelEntriesThatProvide(ctx context.Context, group, version, kind string) (entries []*manifests.ChannelEntry, err error)
	// Get latest channel entries that provide an api
	GetLatestChannelEntriesThatProvide(ctx context.Context, group, version, kind string) (entries []*manifests.ChannelEntry, err error)
