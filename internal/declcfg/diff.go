...
func Diff(oldModel, newModel model.Model) (model.Model, error) {
	...
	if len(oldModel) == 0 {
		...
		for _, outputPkg := range outputModel {
			...
			if !oldHasPkg {
				...
				fmt.Printf("output package %s not in old model\n", outputPkg.Name)
				continue
			}
			...
			if len(outputPkg.Channels) == 0 {
				...
				fmt.Printf("deleting pkg %s from output model\n", outputPkg.Name)
	...
	//
	// TODO(estroz): currently this function does not fill in dependency upgrade graph,
	// just adds the latest bundle(s) that satisfy all dependencies.
	// Those upgrade graphs must be filled in so new bundles that provide new dependencies
	// within the same dependency package can be upgraded to.
	if err := addAllDependencies(newModel, oldModel); err != nil {
		return nil, err
	}
...
func addAllDependencies(srcModel, dstModel model.Model) error {
	// Get every dstModel's bundle's dependencies, and their dependencies, etc. by BFS.
	allProvidingBundles := []*model.Bundle{}
	for curr := getBundles(dstModel); len(curr) != 0; {
		...
		curr = nil
		for _, pkg := range srcModel {
			providingBundles := getProvidingBundles(pkg, reqGVKs, reqPkgs)
	...
	for _, b := range allProvidingBundles {
		// Only add bundles that are not present in dstModel already, and copy packages/channels
		// if necessary; package/channel will not exist in dst if the bundle does not,
		// so no unnecessary copying will occur
		dstPkg, dstHasPkg := dstModel[b.Package.Name]
		if !dstHasPkg {
			dstPkg = copyPackageNoChannels(b.Package)
			dstModel[dstPkg.Name] = dstPkg
		}
		dstCh, dstHasCh := dstPkg.Channels[b.Channel.Name]
		if !dstHasCh {
			dstCh = copyChannelNoBundles(b.Channel, dstPkg)
			dstPkg.Channels[dstCh.Name] = dstCh
		}
		if _, dstHasBundle := dstCh.Bundles[b.Name]; dstHasBundle {
			continue
		}

		dstBundle := copyBundle(b, dstCh, dstPkg)
		dstModel.AddBundle(*dstBundle)
	}
...
// getProvidingBundles returns the latest-version bundles in pkg that provide
